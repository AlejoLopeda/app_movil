<template>
  <section class="monthly-section">
    ...
    <div v-if="errorMessage" class="monthly-error">
      {{ errorMessage }}
    </div>
...
const detailItem = computed(() => {
  ...
  return chartItems.value.find((item) => item.key === activeKey.value) ?? null
})

const balanceClass = computed(() => ({
  'monthly-summary-card--positive': balance.value >= 0,
  'monthly-summary-card--negative': balance.value < 0
}))
...
function onModeChange(event) {
  const next = event.detail?.checked ? 'bar' : 'pie'
  summary.setChartMode(next)
}
...
function toggleCategory(key) {
  summary.toggleCategory(key)
}

function clearFilters() {
  summary.clearCategories()
}

const formatCurrency = (value) => summary.formatCurrency(value)
...
watch(chartItems, (items) => {
  ...
  if (!activeKey.value || !items.some((item) => item.key === activeKey.value)) {
...
.monthly-summary-card--positive {
  background: linear-gradient(135deg, #2a9d8f, #4caf50);
}

.monthly-summary-card--negative {
  background: linear-gradient(135deg, #a63a50, #f07167);
}
